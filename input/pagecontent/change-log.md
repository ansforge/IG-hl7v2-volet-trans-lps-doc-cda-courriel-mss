...
* Revue des exemples de MDN et de courriel standard ([35](https://github.com/ansforge/IG-hl7v2-volet-trans-lps-doc-cda-courriel-mss/issues/35))
  * [Structure du MDN](struct-msg-mdn.html) :
    ...
    * `Content-Transfer-Encoding: 8bit` déclaré sur la partie `message/rfc822` de l'exemple : à défaut elle valait `7bit` alors que son contenu est en `8bit`, et la [RFC 2045 §6.4](https://datatracker.ietf.org/doc/html/rfc2045#section-6.4) n'autorise que `7bit`, `8bit` ou `binary` pour un type composite
    * rappel que les entêtes ne peuvent porter que des caractères US-ASCII, l'entête `Subject:` devant être encodé selon la [RFC 2047](https://datatracker.ietf.org/doc/html/rfc2047) et les paramètres `name` et `filename` selon la [RFC 2231](https://datatracker.ietf.org/doc/html/rfc2231)
    ...
    * mise en conformité du champ `Disposition:` en cas d'erreur : la valeur prescrite jusqu'ici (`processed/Error: <chaîne d'erreur>`) n'est pas conforme à la grammaire de la [RFC 8098 §3.2.6](https://datatracker.ietf.org/doc/html/rfc8098#section-3.2.6), qui n'admet comme modificateur que le littéral `error` ou un `Atom` au sens de la [RFC 5322 §3.2.3](https://datatracker.ietf.org/doc/html/rfc5322#section-3.2.3) — lequel exclut les espaces et le caractère « : » — et contredisait l'ABNF `disposition-modifier = "error"` que le volet rappelait par ailleurs. La valeur devient `processed/error` et la chaîne d'erreur, **inchangée**, est portée par le champ `Error:` ([RFC 8098 §3.2.7](https://datatracker.ietf.org/doc/html/rfc8098#section-3.2.7)), obligatoire en cas d'erreur — **un système qui extrayait la chaîne d'erreur du champ `Disposition:` doit désormais la lire dans le champ `Error:`**
    * correction du système de codage annoncé dans la chaîne d'erreur de l'exemple : le code `902` relève de la table `applicationErrorCondition` ([codes d'erreur](error-codes.html)) et non de `messageErrorCondition`
  ...
  * [Structure d'un message de notification format courriel standard](struct-email-standard.html) :
    ...
    * mêmes corrections d'encodage que dans l'annexe MDN : `Content-Transfer-Encoding: 8bit` sur la partie `message/rfc822`, et rappel de l'encodage RFC 2047 / RFC 2231 des entêtes ; nettoyage des espaces en fin de ligne
    * ajout d'un point d'attention sur l'accès aux documents : le courriel d'origine étant restitué dans une partie `message/rfc822`, son ouverture est nécessaire pour atteindre ses pièces jointes, et le traitement de ce type de partie varie selon les clients de messagerie — point à vérifier à l'intégration, cette annexe couvrant les situations où la notification est reçue au moyen d'un webmail ou d'un client de messagerie standard
