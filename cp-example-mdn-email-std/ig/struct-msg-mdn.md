...
>  **Point d'attention :** la valeur « `processed/Error: code erreur^libellé erreur` » du champ `Disposition:` n'est pas conforme à la grammaire de la [RFC 8098 §3.2.6](https://datatracker.ietf.org/doc/html/rfc8098#section-3.2.6) : un `disposition-modifier` y est défini comme un `Atom` au sens de la [RFC 5322 §3.2.3](https://datatracker.ietf.org/doc/html/rfc5322#section-3.2.3), lequel n'admet ni le caractère « : » ni les espaces. Le code erreur porté par cette valeur n'est donc pas exploitable par un outillage MDN standard, qui ne retiendra que « `processed` ». 
