...
* Dans le contexte du présent volet, ce texte doit au moins contenir, en cas d’erreur, le code et le libellé de l’erreur retournés par le CONSOMMATEUR.
...
>  **Point d'attention :** la grammaire du champ `Disposition:` ([RFC 8098 §3.2.6](https://datatracker.ietf.org/doc/html/rfc8098#section-3.2.6)) n'admet, après le `/`, qu'un modificateur unique : le littéral `error`, ou un `extension-disposition-modifier` défini comme un `Atom` au sens de la [RFC 5322 §3.2.3](https://datatracker.ietf.org/doc/html/rfc5322#section-3.2.3) — jeton qui n'admet ni le caractère « : » ni les espaces. Le texte de diagnostic dispose, quant à lui, de son propre champ `Error:`. La forme retenue par le volet place donc dans le modificateur ce que la RFC répartit sur deux champs : ```Forme retenue par le volet : Disposition: automatic-action/MDN-sent-automatically; processed/Error: 902^Identifiant de patient inconnu Forme conforme à la grammaire de la RFC 8098 : Disposition: automatic-action/MDN-sent-automatically; processed/error Error: 902^Identifiant de patient inconnu``` Un outillage MDN standard ne peut pas interpréter le modificateur de la première forme : il ne retiendra que « `processed` » et le code erreur sera perdu. 

 Cette forme, historiquement retenue par le volet, est conservée. Le champ `Error:` permet aux CONSOMMATEURS s'appuyant sur un outillage MDN standard d'accéder à la même information. 

 La [RFC 8098](https://datatracker.ietf.org/doc/html/rfc8098) demeure la référence pour la structure du MDN ; le présent volet se limite à préciser les informations que le MDN doit véhiculer pour permettre son traitement automatisé par la PFI réceptrice. 
...
L’exemple suivant décrit le MDN (accusé de lecture négatif) généré dans le contexte du cas d’usage « Transmission d’un document clinique d’un patient d’un CH vers un autre CH - Gestion des erreurs » présenté au [paragraphe suivant](volume1.md#description-du-cas-dusage-en-erreur) du présent volet.
